...
SELECT * FROM Customers ORDER BY ContactName DESC; -- 조회결과 ContactName 내림차순
...
-- 연습2) 고객이 있는 나라 오름차순 정렬로 조회(단 한번씩만 결과로 나오도록)
SELECT DISTINCT Country FROM Customers ORDER BY Country;
-- 연습3) 고객이 있는 도시 오름차순 정렬로 조회(단 한번씩만 결과 나오도록)
...
SELECT CustomerID, CustomerName, ContactName, Address, City, PostalCode, Country FROM Customers ORDER BY 5,7;

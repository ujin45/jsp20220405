...
SELECT * FROM Products JOIN Categories; -- Catesian Product
SELECT * FROM Products JOIN Categories ON Products.CategoryID = Categories.CategoryID;

-- Products 테이블과 Suppliers 테이블 SupplierID 컬럼이 같은 것 끼리 INNER JOIN 한 결과 조회
...
SELECT * FROM Customers c JOIN Suppliers s ON c.Country = s. Country ORDER BY c.Country; -- 교집합 조회
